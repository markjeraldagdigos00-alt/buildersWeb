...
app.get('/api/scanner/dashboard', async (req, res) => {
  ...
  const timeInRes = await pool.query('SELECT COUNT(*) as count FROM attendance WHERE date = $1 AND (time_in_am IS NOT NULL OR time_in IS NOT NULL)', [today]);
  const timeOutRes = await pool.query('SELECT COUNT(*) as count FROM attendance WHERE date = $1 AND (time_out_pm IS NOT NULL OR time_out IS NOT NULL)', [today]);
...
app.post('/api/scanner/scan', async (req, res) => {
  ...
  if (attRes.rows.length === 0) {
    await pool.query('INSERT INTO attendance (worker_id, date, time_in_am, time_in, status) VALUES ($1, $2, NOW(), NOW(), $3)', [worker.worker_id, today, 'Present']);
    res.json({ success: true, status_type: 'TIME IN (AM)', name: worker.full_name, position: worker.position, time: currentTime });
  } else {
    const att = attRes.rows[0];
    if (!att.time_out_am) {
      await pool.query('UPDATE attendance SET time_out_am = NOW() WHERE id = $1', [att.id]);
      res.json({ success: true, status_type: 'TIME OUT (AM)', name: worker.full_name, position: worker.position, time: currentTime });
    } else if (!att.time_in_pm) {
      await pool.query('UPDATE attendance SET time_in_pm = NOW() WHERE id = $1', [att.id]);
      res.json({ success: true, status_type: 'TIME IN (PM)', name: worker.full_name, position: worker.position, time: currentTime });
    } else if (!att.time_out_pm) {
      await pool.query('UPDATE attendance SET time_out_pm = NOW(), time_out = NOW() WHERE id = $1', [att.id]);
      res.json({ success: true, status_type: 'TIME OUT (PM)', name: worker.full_name, position: worker.position, time: currentTime });
    } else {
      res.status(400).json({ error: `${worker.full_name} ay nakumpleto na ang Time In at Time Out ngayong araw.` });
    }
  }
});
...
app.get('/api/worker/:id', async (req, res) => {
  ...
  try {
    ...
    const daysWorkedRes = await pool.query('SELECT COUNT(*) FROM attendance WHERE worker_id = $1 AND (time_in_am IS NOT NULL OR time_in IS NOT NULL)', [worker.worker_id]);
        ...
        <section id="tab-today" class="hidden bg-slate-800 p-4 rounded-2xl space-y-3 border border-slate-700 shadow-xl">
          ...
          <div class="overflow-x-auto max-h-[55vh]">
            <table class="w-full text-left text-xs">
              <thead><tr class="bg-slate-700 text-slate-300 border-b border-slate-600"><th class="p-2">Worker</th><th class="p-2">Pos</th><th class="p-2">In (AM)</th><th class="p-2">Out (PM)</th></tr></thead>
      ...
      <script>
        ...
        async function loadToday() {
          try {
            ...
            data.forEach(i => {
              let inVal = i.time_in_am || i.time_in;
              let outVal = i.time_out_pm || i.time_out;
              tbody.innerHTML += '<tr><td class="p-2 font-semibold">' + i.full_name + '</td><td class="p-2 text-slate-300">' + i.position + '</td><td class="p-2 text-blue-400 font-bold">' + (inVal?new Date(inVal).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'-') + '</td><td class="p-2 text-purple-400 font-bold">' + (outVal?new Date(outVal).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'—') + '</td></tr>';
            });
          } catch(e) {}
        }
        loadDashboard();
    ...
    <body class="bg-slate-900 text-white min-h-screen flex flex-col md:flex-row">
      ...
      <main class="flex-1 p-4 overflow-y-auto">
        ...
        <section id="adm-qr" class="hidden bg-slate-800 p-4 rounded-xl border border-slate-700 space-y-3">
          ...
          <div class="overflow-x-auto"><table class="w-full text-left text-xs">
            <thead><tr class="bg-slate-700 text-slate-300 border-b border-slate-600"><th class="p-2">Worker Name</th><th class="p-2">Date</th><th class="p-2">In (AM)</th><th class="p-2">Out (PM)</th><th class="p-2">Status</th></tr></thead>
      ...
      <script>
        ...
        async function loadAttendance() {
          try {
            ...
            data.forEach(a => {
              let inVal = a.time_in_am || a.time_in;
              let outVal = a.time_out_pm || a.time_out;
              tbody.innerHTML += '<tr><td class="p-2 font-semibold">' + a.full_name + '</td><td class="p-2">' + a.date + '</td><td class="p-2 text-blue-400">' + (inVal ? new Date(inVal).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) : '-') + '</td><td class="p-2 text-purple-400">' + (outVal ? new Date(outVal).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) : '—') + '</td><td class="p-2 text-emerald-400">' + a.status + '</td></tr>';
            });
          } catch(err) {}
        }
          ...
          try {
            ...
            if(res.ok) {
              ...
              if(data.today_attendance) {
                ...
                let timeInStr = inVal ? new Date(inVal).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) : '-';
              ...
              if(data.attendance.length === 0) {
                ...
                data.attendance.forEach(att => {
                  let inVal = att.time_in_am || att.time_in;
                  let outVal = att.time_out_pm || att.time_out;
